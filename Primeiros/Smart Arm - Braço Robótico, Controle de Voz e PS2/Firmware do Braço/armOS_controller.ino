...
//Turning  the hand tool (currently it's a lantern) on will save to eeprom servo position
/*
 * This code uses Thread Arduino Library for fake parallelism
 * It enables a repeating code block upon Serial command (like incrementing servo angle or stepping a stepper) and disables it upon another Serial command 
 * The thread runs a callback every X number of seconds
 */
...
const int EEPROM_SERVO_ADDR[] = {0,1,2};
...
for(int i=0;i<servoNum;i++){
   int val  = EEPROM.read(EEPROM_SERVO_ADDR[i]);
   if(val >=angleSet[0] and val<= angleSet[1])
    servoPos[i]=val;
}
...
void save_servo_pos(){
        for(int i=0;i<servoNum;i++)
      EEPROM.update(EEPROM_SERVO_ADDR[i], servoPos[i]);
}
