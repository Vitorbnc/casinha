...
Thread thrAccelerate,thrReadSonars;
...
//confidence values from 0 to 100 ( not using 0.0 to 1.0 with float for speed reasons )
int frontConfidence, rearConfidence;
const int minConfidence = 65;


//int sonarTries = 3;
int sonarReadings_count = 3;
// Max deviation in form:  Md(m)= 0.1m +1, where m is mean (expected value)
float maxDeviation_coefficient= 0.1;
int maxDeviation_constantTerm = 2;
...
bool hasFrontSonar = true;
bool hasRearSonar = true;
// NewPing sonar (trigger,echo,maxDistance)
const int frontSonar_count = 2;
NewPing frontSonar[frontSonar_count]={NewPing (27, 26, maxDistance),NewPing (29,28,maxDistance)};
//NewPing frontSonar_right(29,28, maxDistance);
NewPing rearSonar(25, 24, maxDistance);

// BPM180 or BMP085 pressure and temp sensor
Adafruit_BMP085 bmp;
...
const int accel_pin =6; // constantes dos pinos dos servos
const int rev_pin = 4; // pino do relé da ré
...
long interval_readSonars = 50;
bool sonarRequested=true;
...
public:
int lastSpeed;
int minMotionSpeed = 4;
int accelIsPositive = true;
bool canMoveForward = true;
bool canMoveBackward = true;

void init(int accel_pin, int rev_pin){
        revpin = rev_pin;
        pinMode(revpin, OUTPUT); // inicializa pino do relé da ré
        accel.attach(accel_pin,800,1800); // attaches the esc to its pin
...
private:
int safeDistance = 85;
int approachDistance = 60;
int criticalDistance = 25; // actual "critical" distance is about 5, setting higher for safety
int safeSpeed = 3; // was 4
int approachSpeed =2;

public:
bool intention_accelerate = false;
void safeRide(){
        if(hasFrontSonar&&engine.lastSpeed>0) {
                if(frontConfidence>minConfidence) {
                  //safeSpeed = 1+(frontDistance/25);
                        if(frontDistance>safeDistance) {
                                if(intention_accelerate) thrAccelerate.enabled = true;
                        }
                        else{
                                if(thrAccelerate.shouldRun()) {
                                        intention_accelerate =true;
                                        thrAccelerate.enabled = false;
                                }


                                if(frontDistance<criticalDistance)  {
                                  intention_accelerate =false;
                                  engine.fastBrake();
                                }

                                //else if(frontDistance<approachDistance) if(engine.lastSpeed>approachSpeed) engine.setSpeed(approachSpeed);

                                else if(engine.lastSpeed>safeSpeed) engine.setSpeed(safeSpeed);
        ...
        if(hasRearSonar&&engine.lastSpeed<0) {
                //  usb.println("called safeRide, has Back SOnar");
                if(rearConfidence>minConfidence) {
                        //  usb.println("safing back");
                      //  safeSpeed = (rearDistance/25)+1;
                        if(rearDistance>safeDistance) {
                                //  usb.println("all good");
                                if(intention_accelerate) thrAccelerate.enabled = true;
                        }
                        else{
                                if(thrAccelerate.shouldRun()) {
                                ...
                                if(rearDistance<criticalDistance+7) {
                                  intention_accelerate =false;
                                 engine.fastBrake();
                               }

                              //  else if(rearDistance<approachDistance) if(engine.lastSpeed<(-approachSpeed)) engine.setSpeed(-approachSpeed);
...
void setupThreads(){
        ...
        thrReadSonars.onRun(readSonarsCallback);

        thrAccelerate.enabled=false;

        thrReadSonars.setInterval(interval_readSonars);
        thrReadSonars.enabled=false;

        thrController.add(&thrAccelerate);
        thrController.add(&thrReadSonars);
}

intTuple readSonar_simple(NewPing* sonar){
  int confidence = 0;
  int distance;
  for(int i=0;i<3;i++){
  unsigned long pulse_uS = sonar->ping();
  distance =  int (pulse_uS/US_ROUNDTRIP_CM);
...
//usb.println(confidence);

  intTuple data(distance,confidence);
  return data;

}

intTuple readSonar(NewPing * sonar){
        int distance;
        int confidence =0;
        int sum=0;
        int sonarReadings[sonarReadings_count];
        for(int i=0; i<sonarReadings_count; i++) {
                unsigned long pulse_uS = sonar->ping();
                sonarReadings[i] =  int (pulse_uS/US_ROUNDTRIP_CM);
                sum+=sonarReadings[i];
                delayMicroseconds(1);
        }
        int mean = sum/sonarReadings_count;
        int deviations[sonarReadings_count];
        //int maxDeviation = maxDeviation_constantTerm + mean*maxDeviation_coefficient;
        int maxDeviation=5;
        for(int i=0; i<sonarReadings_count; i++) {
                deviations[i] = abs(mean-sonarReadings[i]);
                if(deviations[i]<=maxDeviation) confidence +=(100/sonarReadings_count);
        }
        distance = mean;
        if(distance<minDistance) confidence = 0;
        /*  usb.println(maxDeviation);
           for(int i=0;i<sizeof(deviations)/sizeof(int);i++){
           usb.println(deviations[i]);
           }
         */
        intTuple data(distance,confidence);
        return data;
}
void readSonarsCallback(){
        if(hasFrontSonar) {
                //frontDistance = (pulse_uS/2) / 29.1;
                //unsigned long  dist =  pulse_uS/US_ROUNDTRIP_CM;
                frontDistance = 5000;
                frontConfidence =0;
                for(int i=0;i<frontSonar_count;i++){
                //  NewPing tmp = frontSonar[i];
                  intTuple frontData= readSonar_simple(&frontSonar[i]);
                  if(frontData.data[1]>minConfidence) {
                  frontDistance = min(frontData.data[0],frontDistance);
                  frontConfidence = max(frontConfidence,frontData.data[1]) ;
                }
              }

                if(debug && sonarRequested) {
                        String tmp="Front--> Distance: "; tmp+= frontDistance; tmp+="cm;  "; tmp+="Confidence: "; tmp+=frontConfidence;
                        if(hasRearSonar) usb.print(tmp);
                        else usb.println(tmp);
                }
        }
// ------------------------------
//-----------------Rear------------------
        if(hasRearSonar) {
                intTuple rearData= readSonar_simple(&rearSonar);
                rearDistance = rearData.data[0];
                rearConfidence = rearData.data[1];
                if(debug && sonarRequested) {
                        String tmp="Rear--> Distance: "; tmp+= rearDistance; tmp+="cm;  "; tmp+="Confidence: "; tmp+=rearConfidence;
                        usb.println(tmp);
                }
...
void setup()
{
        steering.init(trn_pin);
        engine.init(accel_pin,rev_pin);
        ...
        if(strcmp(dir[0].c_str(),"ctl")==0) {
                ...
                if(strcmp(dir[1].c_str(),"LJ")==0) {
                        ...
                        if(intArg[0]==3) {
                          ...
                          if(thrReadSonars.shouldRun()){
                             thrReadSonars.enabled = false;
                             engine.minMotionSpeed=5;
                            // printValue("ctl.vibrate",255);
                             blinkOK();
                            // delay(600);
                             //printValue("ctl.vibrate",0);
                           }
                          else {
                          thrReadSonars.enabled=true;
                          engine.minMotionSpeed=4;
                          //printValue("ctl.vibrate",85);
                          blinkOK();
                          //printValue("ctl.vibrate",0);
                        }
                }
                ...
                if(strncmp(dir[1].c_str(),"temperature",4)==0) {
                        int temp = bmp.readTemperature();
                        printValue("temperature(C)",temp);
                }
                else if (strncmp(dir[1].c_str(),"pressure",4)==0) {
                        float temp = bmp.readPressure();
                        printValue("pressure(Pa)",temp);
                }
        }
