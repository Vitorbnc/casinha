 ...
 * This project builds upon multiScope_v2, which was based on STM32-O-Scope.
 * It's gonna comprise some useful tools:
 *  Oscilloscope
 *  Inductance Meter
 *  Waveform Generator (maybe)
 *  Temperature and pressure sensor (extra)
...
const int8_t SCOPE_IN_PIN = PB0;   // Analog input pin: any of LQFP44 pins (PORT_PIN), 10 (PA0), 11 (PA1), 12 (PA2), 13 (PA3), 14 (PA4), 15 (PA5), 16 (PA6), 17 (PA7), 18 (PB0), 19  (PB1)
...
const int INPUT_FALL_PIN = PA15;
const int INPUT_RISE_PIN = PA12;
...
void setup()
{
  ...
  #define  TEST_WAVE_FREQ 1000 //1kHz
        ...
        pinMode(SCOPE_IN_PIN, INPUT_ANALOG); // this is the scope wave input
        pinMode(INPUT_FALL_PIN, INPUT); // this is tied to scope wave input too(via 2.2k resistor), but is used to count frequency
        pinMode(INPUT_RISE_PIN, INPUT); // this is tied to scope wave input too(via 2.2k resistor), but is used to count frequency
...
void freqCounterInit(){
  attachInterrupt(INPUT_FALL_PIN,fallCounterISR,FALLING);
  attachInterrupt(INPUT_RISE_PIN,riseCounterISR,RISING);
...
//TODO: inidicate working range as above 1kHz
  ...
  //attachInterrupt(SCOPE_IN_PIN,cyclesCounterISR,RISING);
...
void setADCs ()
{
        //  const adc_dev *dev = PIN_MAP[SCOPE_IN_PIN].adc_device;
        int pinMapADCin = PIN_MAP[SCOPE_IN_PIN].adc_channel;
...
void triggerAlways(){
      ...
      triggerPoints[0] = analogRead(SCOPE_IN_PIN);
      notTriggered=false;
      triggerPoints[0] = triggerPoints[1]; //analogRead(SCOPE_IN_PIN);
...
void triggerBoth()
{
        triggerPoints[0] = analogRead(SCOPE_IN_PIN);
        while(notTriggered) {
                triggerPoints[1] = analogRead(SCOPE_IN_PIN);
